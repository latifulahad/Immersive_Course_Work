...
#
# Table name: tracks
# album        :string       not null           - - f_key == asin FROM albums
...
def expensive_tastes
  ...
  execute(<<-SQL)
  select
    style, (sum(album_stat.price) / sum(album_stat.count))
  from
    styles
  join (
    select
    ...
    group by
      asin
  ) as album_stat on album_stat.asin = styles.album
  group by
    style
  order by
    (sum(album_stat.price) / sum(album_stat.count)) DESC, style ASC
  limit
    5
    SQL
  end
